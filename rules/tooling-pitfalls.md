...
| 19 | **配置流断裂（声明→解析→传递→读取→使用任一环断开）**（来源：文档审查套件 STR-004 在 4 节点断裂；cross-project-synthesis #4）→ 规则/参数在配置中声明但链路某节点静默失效，报错时已在用点 | 防御模式：回退路径的 config 键集**必须与主路径一致**（文档审查套件 `_resolve_auditor_config()` 注释"陷阱 #4：回退路径 config 键集必须与 build_auditors 一致"）；新增注册点后立即用 `verify-registries.py`（档 A-A1）断言多注册表键集一致 |
| 20 | **ruff per-file ignore 无理由注释**（来源：工程分析套件 pyproject.toml）→ 后人 copy-paste 忽略规则，无上下文 | 每条 per-file-ignores/noqa 必须带中文理由注释（如 `"N806" 大写变量是有意的 SPC 常量 A2/D3/D4`）；新增规则类别需先确认非"覆盖问题"而是"约定豁免" |
