...
| 20 | **自校验正则误伤 docstring/注释中的反例教学文字**（如「禁止自校验 check(name, X, X)」）→ quality-gate 必红 | 自校验扫描跳过注释/docstring 行（`line.lstrip().startswith(('#', '"', "'"))`）；或将反例改写为不匹配形式（`check(name, X, Y)` + 说明）。**2026-08-15 复发**：ci.yml「提交前自检」裸 grep `except:` 命中 verify-docs.py docstring 教学文字（Max 审查修复注释引入；quick-check 必红且连累所有下游项目，8-14 基线无此注释故当时通过）→ 修复：quick-check 改调 `verify-docs.py --check-bare-handlers`（语义检查向量 1 单一信源，注释感知），裸 grep 从 CI 移除 |
